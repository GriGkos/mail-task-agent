...
В карточке задачи показываются отправитель, тема и дата исходного письма. Кнопка
**Открыть письмо** ведёт напрямую в Gmail/Outlook, а для IMAP открывает защищённую
страницу MailTaskBot, которая загружает исходное письмо из подключённого ящика.
